...
**Шардирование:**
- Данные будут шардироваться по `user_id` для равномерного распределения нагрузки
- Планируется 16 шардов, что позволит масштабироваться до значительных объёмов
- Каждый шард будет иметь master для записи и read-реплики для чтения

**Кэширование: Redis**

Для обработки 100,000 RPS на чтение необходим кэш, чтобы не перегружать основную БД:
...
- **Zone Cache**: кэширование параметров тарифных зон с TTL 10 минут (для работы при недоступности `zone` сервиса)

**Cold Storage: S3**

Для архивных данных (старше 2 дней) используется объектное хранилище:
...
- Синхронная репликация на одну standby-реплику для критичных данных
