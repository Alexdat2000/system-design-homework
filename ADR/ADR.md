...
**Выбор: Redis для офферов, кэш для заказов**
...
**Рассмотренные альтернативы генерации order_id:**

1. **Клиентское приложение генерирует UUID** (выбрано)
...
2. **Наш сервис генерирует ID (sequence/auto-increment/UUID)**
...
4. **Создание оффера** (желательно для оптимизации)
   - Механизм: проверка кэша Redis перед созданием - если оффер с такими параметрами (user_id, scooter_id) уже существует и валиден (не истёк, не использован), возвращаем существующий из кэша
...
Речь идёт о репликации базы данных PostgreSQL (шардов с заказами и транзакциями). Каждый шард PostgreSQL имеет master для записи и реплики для чтения и отказоустойчивости.

Синхронная репликация на 2 standby-реплики используется для операций, где потеря данных недопустима:
...
**Реализация:**
- Синхронная репликация на 2 standby-реплики для операций создания/завершения заказа и сохранения транзакций
